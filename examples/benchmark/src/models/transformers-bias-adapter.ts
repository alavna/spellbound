...
async function getBiasClassifier(
  modelId: string,
...
/** Bias detection adapter — supports binary bias detector and multi-class bias type classifier */
export class TransformersBiasAdapter implements ModelAdapter {
  readonly type = 'transformers-bias';
  ...
  async classify(text: string, model: string, _labels: string[], _signal?: AbortSignal): Promise<ClassificationResult> {
    const classifier = await getBiasClassifier(model, this.onProgress);
